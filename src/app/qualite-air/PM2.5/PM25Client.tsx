...
export default function PM25Client() {
    ...
    const fetchPM25Data = async () => {
      try {
        ...
        if (data.data?.[0]?.sous_indices) {
          const pm25Data = data.data[0].sous_indices.find(
            indice => indice.polluant_nom === 'PM2.5'
          );
          
          if (pm25Data) {
            setPollutantData(pm25Data);
          } else {
            throw new Error('Données PM2.5 non trouvées');
          }
        } else {
      ...
      console.error('Erreur lors du chargement des données:', error);
        setError('Impossible de récupérer les données PM2.5');
      } finally {
        setLoading(false);
      }
    };
    ...
    return (
      <div className="min-h-screen bg-[#F8F7F4] py-4 sm:py-8 px-4 sm:px-6 lg:px-8">
        <div className="max-w-4xl mx-auto text-center">
          <p className="text-lg sm:text-xl text-black">Chargement des données PM2.5...</p>
        </div>
      </div>
    );
  }
      ...
      <div className="max-w-4xl mx-auto">
        ...
        >
          <p className="text-lg sm:text-xl md:text-2xl lg:text-3xl font-light text-black leading-tight">
            Aujourd&apos;hui à Lyon le niveau de PM2.5 est{' '}
            <span 
          ...
          <div className="backdrop-blur-sm rounded-2xl p-4 sm:p-6 shadow-lg border border-gray-100">
            ...
            <div className="flex items-center justify-between gap-4 mb-6">
              <div className="flex-1">
                ...
                <p className="text-gray-600 text-sm sm:text-base">Particules PM2.5 mesurées</p>
              </div>
        ...
        >
          <div className="backdrop-blur-sm rounded-2xl p-4 sm:p-6 shadow-lg border border-gray-100">
            ...
            <p className="text-black opacity-80 leading-relaxed text-sm sm:text-base">
              Particules fines ou « poussières » de diamètre inférieur à 2,5 μm (microns).
            </p>
          </div>
        </motion.div>
        ...
        >
          <div className="backdrop-blur-sm rounded-2xl p-4 sm:p-6 shadow-lg border border-gray-100">
            ...
            <p className="text-black opacity-80 leading-relaxed text-sm sm:text-base">
              Les pics de concentrations s&apos;observent principalement en hiver, à proximité des axes routiers majeurs ou dans les bassins résidentiels utilisant largement des appareils ou méthodes de chauffage au bois non performant.
            </p>
          </div>
        </motion.div>
        ...
        >
          <div className="backdrop-blur-sm rounded-2xl p-4 sm:p-6 shadow-lg border border-gray-100">
            ...
            <p className="text-black opacity-80 leading-relaxed text-sm sm:text-base">
              Comme elles sont plus petites, les particules PM2.5 pénètrent plus profondément dans les voies respiratoires et atteignent les alvéoles pulmonaires. À ce titre, on leur attribue un plus grand impact sanitaire.
            </p>
          </div>
        </motion.div>
        ...
        >
          <div className="backdrop-blur-sm rounded-2xl p-4 sm:p-6 shadow-lg border border-gray-100">
            ...
            <div className="text-black opacity-80 leading-relaxed space-y-1 text-sm sm:text-base">
              <p>• Trafic automobile (échappements, freinage, usure des pneus)</p>
              <p>• Chauffage au bois domestique non performant</p>
              <p>• Activités industrielles</p>
              <p>• Agriculture (particules secondaires)</p>
              <p>• Phénomènes naturels (érosion, pollens)</p>
            </div>
          </div>
        </motion.div>
      </div>
