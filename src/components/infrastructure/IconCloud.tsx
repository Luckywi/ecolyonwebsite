...
export function IconCloud({ icons, images, iconSize = 80 }: IconCloudProps) {
      ...
      if (offCtx) {
        if (images) {
          ...
          img.onload = () => {
            ...
            // Dessiner l'image avec les bonnes proportions (sans clipping circulaire)
  ...
  // Generate initial icon positions on a sphere (rayon doublé)
  useEffect(() => {
    const items = icons || images || [];
    const newIcons: Icon[] = [];
    const numIcons = items.length || 20;

    // Fibonacci sphere parameters avec rayon doublé
    ...
    for (let i = 0; i < numIcons; i++) {
      ...
      newIcons.push({
        x: x * 200, // Doublé de 100 à 200
        y: y * 200, // Doublé de 100 à 200
        z: z * 200, // Doublé de 100 à 200
        scale: 1,
        opacity: 1,
        id: i,
      });
  ...
  const handleMouseDown = (e: React.MouseEvent<HTMLCanvasElement>) => {
    ...
    iconPositions.forEach((icon) => {
      ...
      const scale = (rotatedZ + 400) / 600; // Ajusté pour le nouveau rayon
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      iconPositions.forEach((icon, index) => {
        ...
        const scale = (rotatedZ + 400) / 600; // Ajusté pour le nouveau rayon
        const opacity = Math.max(0.2, Math.min(1, (rotatedZ + 300) / 400)); // Ajusté pour le nouveau rayon
